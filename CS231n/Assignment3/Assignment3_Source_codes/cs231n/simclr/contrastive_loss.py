...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j) / (z_i_norm * z_j_norm)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator1 = denominator1 - torch.exp(sim(z_k, z_k) / tau)
        denominator2 = denominator2 - torch.exp(sim(z_k_N, z_k_N) / tau)

        loss1 = -torch.log(numerator/denominator1)
        loss2 = -torch.log(numerator/denominator2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot = torch.sum(out_left * out_right, dim=1, keepdims=True)

    pos_pairs = dot / (left_norm * right_norm)
...
def compute_sim_matrix(out):
    ...
    N = int(out.shape[0] / 2)
    dot_matrix = out.matmul(out.T)
    norm_vertical = torch.linalg.norm(out, dim=1, keepdims=True)
    norm_horizontal = norm_vertical.view(1, -1)
    sim_matrix = dot_matrix / norm_vertical / norm_horizontal
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    positive_pairs = torch.cat([sim_pos, sim_pos], dim=0)
