...
def rnn_step_backward(dnext_h, cache):
    ...
    dx, dWx, db = affine_backward(dsum, (x, Wx, b))
    dprev_h, dWh, db = affine_backward(dsum, (prev_h, Wh, b))
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[0]):
      next_h, data = rnn_step_forward(x[t], prev_h, Wx, Wh, b)
      cache.append(data)
      h[t] = next_h
      prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      ...
      dh_sum = dh[:,t,:] + dprev_h
      dnext_h = data[-1]
      dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dh_sum, data)
      dWx += dWx_
      dWh += dWh_
      db += db_
      dx[:,t,:] = dx_
...
def word_embedding_backward(dout, cache):
    ...
    dW = np.zeros_like(W)
    x_flat = x.reshape(-1)
    dout_flat = dout.reshape(-1, dout.shape[2])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dprev_c, df, di, dg = dnext_c * (f, prev_c, g, i)

    dai = di * i*(1-i)    
    daf = df * f*(1-f)
    dao = do * o*(1-o)
    dag = dg * (1 - np.tanh(a_split[3])**2)

    da = np.hstack((dai, daf, dao, dag))

    dx, dWx, _ = affine_backward(da, (x, Wx, np.zeros(Wx.shape[1])))
    dprev_h, dWh, _ = affine_backward(da, (prev_h, Wh, np.zeros(Wh.shape[1])))
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      ...
      if t == 0:
        res = lstm_step_forward(x[:, t, :], h0, c0, Wx, Wh, b)
        
      else:
        res = lstm_step_forward(x[:, t, :], h[:, t-1, :], c[:, t-1, :], Wx, Wh, b)
