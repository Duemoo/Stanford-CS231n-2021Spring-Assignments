...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        #reference: https://chioni.github.io/posts/transformer2/#TransformerEncoderLayer
        pe = torch.zeros(max_len, embed_dim)
        i_term = torch.arange(0, max_len).float().unsqueeze(1)
        j_term = torch.exp(torch.arange(0, embed_dim, 2).float() * -(math.log(10000.0) / embed_dim))
        pe[:, 0::2] = torch.sin(i_term * j_term)
        pe[:, 1::2] = torch.cos(i_term * j_term)
        pe = pe.unsqueeze(0)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.H = num_heads
        self.D_H = int(embed_dim / num_heads)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.H
        query_split = self.query(query).view(N, -1, H, self.D_H).transpose(1,2)
        key_split = self.key(key).view(N, -1, H, self.D_H).transpose(1,2)
        value_split = self.value(value).view(N, -1, H, self.D_H).transpose(1,2)

        scores = query_split.matmul(key_split.transpose(-2, -1)) / math.sqrt(self.D_H)
        ...
        scores = scores.matmul(value_split)
        scores = scores.transpose(1,2).contiguous().view(N, -1, D)
